...
function listarComputadoras(){
    ...
    $("#productos").empty();
    $.ajax({
        url: "http://localhost/act2_angelyahir/public/api/computadorasApi",
        type: "POST",
        success: function(result){
            var resultado = JSON.parse(result);
            resultado.forEach(renglon => {
                $registro = `<div class="row z-depth-2 section contProductos">
                    ...
                    <div class="col s12 m8 l9">
                        <a href="computadoras/${renglon.nombre}"><h4 class="black-text">${renglon.nombre}</h4></a>
                        <h5>$ ${renglon.precio}</h5>
                        <h6>Marca: ${renglon.marca}</h6>
                        <h6>Modelo: ${renglon.modelo}</h6>
                    </div>
